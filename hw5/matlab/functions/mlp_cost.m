function [cost, grad, pred_prob] = mlp_cost(weight, ei, X, y, prediction_on)
    ...
    %%%%%%%%%%%%%%
      ...
      if l>1
          activation{l} = wStack{l}.W * activation{l-1} + repmat(wStack{l}.b, 1,n);
      end
    ...
    ceCost = 0;
    Y = log(activation{L});
    % linearInd = sub2ind(matrixSize, rowSub, colSub) returns
    % the linear index equivalents to the row and column subscripts 
    % rowSub and colSub for a matrix of size matrixSize.
